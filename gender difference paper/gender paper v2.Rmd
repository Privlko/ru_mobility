...
Regarding wages, we consider a combination hourly net wages from the respondent's primary job.
We create this measure by dividing monthly wages (IXWAGELM) by weekly working time (IXPWRKWH). 
This measure is a rough ratio of hourly wages; if respondents are paid in a different currency they are asked to convert the monthly amount to rubles. 
...
In addition, we will control for changes in marital status (IXMARIST) in an effort to anticipate changing social roles which may impact wages.
...
We will only include these firm-level measures into the multinomial logistic regression, because these factors do not change over time.
...
Overall, respondents are largely immobile although women (0.813) are less mobile compared to men (0.760). 
...
Respondents are much more likely to list exiting a firm, with men (0.184) listing more exits than women (0.130).
...
m1 <- multinom(mob_final ~  gender + age  + tenure + factor(super) + factor(esec_simple) +factor(marr) + hours + factor(size) +factor(gov), data=d1)
m2 <- multinom(mob_final ~  gender + age  + tenure + factor(super) + factor(esec_simple) +factor(marr) + hours + factor(size) +factor(gov) , data=d2)
...
kable(cbind(t1, t2), 
  ...
  pack_rows("Promotion", 1, 13) %>% 
  pack_rows("Lateral change", 14, 26) %>% 
  pack_rows("Exit", 27, 39) %>% 
