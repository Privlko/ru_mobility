...
load("C:/Users/ivan.privalko/Documents/RProjects/ru_mobility/genderpaper.RData")
...
Blau and Duncan [-@blau1981race] find that young women are more likely to exit a firm when compared to young men in a sample of US workers. 
Using more recent ut similar data, Hachen [-@hachen1988gender] finds the opposite result, women are less likely to exit a firm when compared to men. 
...
More recent data suggests that gender differences in promotion are slight, once job seeking efforts are considered. 
For example, Bosquet et al., [-@bosquet2019gender] explore gender differences in chances of promotion using a sample of French economists.
They note that female economists are less likely to be promoted when compared to their male counterparts, but this difference is not statistically significant. 
Further, the authors note that women are significantly less likely to seek out promotion, which suggests they are less likely to job search. 
This gap in promotion seeking may be a key factor in wage and occupational attainment gaps, which occur later in women's careers.
Elsewhere, Hospido et al., [-@hospido2019gender] use job history data for economists at the European Central Bank. 
The authors find a gender gap in promotion prior to 2010, after 2010 however, this gap disappears most likely due to internal efforts to close gender differences and promote diversity.
Once again, the authors note that women are significantly less likely to apply for promotion when compared to men, and although there was no difference in the likelihood of promotion genders, this lack of application likely plays a part in earnings differences which occur later. 
...
In general, the literature suggests that differences between genders in their likelihood of mobility are uncommon and when they exist, they can often be explained by additional measures, such as the type of job mobility experienced.
...
The multinomial logistic regression considers the controls discussed above, as well as gender and age. 
...
m1 <- multinom(mob_final ~  gender + age  + tenure + super + factor(esec_simple) +factor(marr) + hours + factor(size), data=d1)
m2 <- multinom(mob_final ~  gender + age  + tenure + super + factor(esec_simple) +factor(marr) + hours + factor(size), data=d2)
...
kable(cbind(t1, t2), 
  ...
  #pack_rows("Promotion", 1, 11) %>% 
  #pack_rows("Lateral change", 12, 22) %>% 
  #pack_rows("Exit", 23, 33) %>% 
...
pm_m <- plm(ln_hrwage ~ 1 + mob_final + age + tenure + super + factor(esec_simple) +factor(marr) + firm_size, 
                   data = df_m,
                   index = c("idind", "round"), 
                    model = "within")
...
pm_f <- plm(ln_hrwage ~ 1 +  mob_final + age + tenure + super + factor(esec_simple) +factor(marr) + firm_size, 
                   data = df_f,
                   index = c("idind", "round"), 
                    model = "within")
...
pm_m <- plm(ln_hrwage ~ mob_final + age + tenure + super + factor(esec_simple) +factor(marr) + firm_size, 
                   data = df_m,
                   index = c("idind", "round"), 
                    model = "within")
...
pm_f <- plm(ln_hrwage ~ mob_final + age + tenure + super + factor(esec_simple) +factor(marr) + firm_size, 
                   data = df_f,
                   index = c("idind", "round"), 
                    model = "within")
